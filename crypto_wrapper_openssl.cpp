...
bool CryptoWrapper::hmac_SHA256(IN const BYTE* key, size_t keySizeBytes, IN const BYTE* message, IN size_t messageSizeBytes, OUT BYTE* macBuffer, IN size_t macBufferSizeBytes)
{
	...
	HMAC(EVP_sha256(), key, keySizeBytes, message, messageSizeBytes, macBuffer, &len);

	return len == HASH_SIZE_BYTES;
}
	...
	OUT BYTE* ciphertextBuffer, IN size_t ciphertextBufferSizeBytes, OUT size_t* pCiphertextSizeBytes)
{
	...
	if (1 != EVP_EncryptUpdate(ctx, NULL, &len, aad, aadSizeBytes)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}

	if (1 != EVP_EncryptUpdate(ctx, ciphertextBuffer + IV_SIZE_BYTES, &len, plaintext, plaintextSizeBytes)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}
	ciphertext_len = len;

	if (1 != EVP_EncryptFinal_ex(ctx, ciphertextBuffer + IV_SIZE_BYTES + ciphertext_len, &len)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}
	ciphertext_len += len;

	if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, GMAC_SIZE_BYTES, ciphertextBuffer + IV_SIZE_BYTES + ciphertext_len)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}
	ciphertext_len += GMAC_SIZE_BYTES;

	memcpy(ciphertextBuffer, iv, IV_SIZE_BYTES);
	*pCiphertextSizeBytes = IV_SIZE_BYTES + ciphertext_len;
...
{
	if (ciphertextSizeBytes < IV_SIZE_BYTES + GMAC_SIZE_BYTES) {
		return false;
	}
	...
	const BYTE* encryptedText = ciphertext + IV_SIZE_BYTES;
	int encryptedTextLen = ciphertextSizeBytes - IV_SIZE_BYTES - GMAC_SIZE_BYTES;
	...
	if (1 != EVP_DecryptUpdate(ctx, NULL, &len, aad, aadSizeBytes)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}

	if (1 != EVP_DecryptUpdate(ctx, plaintextBuffer, &len, encryptedText, encryptedTextLen)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}
	plaintext_len = len;

	if (1 != EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, GMAC_SIZE_BYTES, (void*)tag)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}

	if (1 != EVP_DecryptFinal_ex(ctx, plaintextBuffer + plaintext_len, &len)) {
		EVP_CIPHER_CTX_free(ctx);
		return false;
	}
	plaintext_len += len;
